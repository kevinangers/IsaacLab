...
def vial_position_in_world_frame(
    env: ManagerBasedRLEnv,
    vial_cfg: SceneEntityCfg = SceneEntityCfg("vial"),
) -> torch.Tensor:
    """The position of the vial in the world frame."""
    ...
    return vial.data.root_pos_w


## Might need when using multiple vials
...
def vial_orientation_in_world_frame(
    env: ManagerBasedRLEnv,
    vial_cfg: SceneEntityCfg = SceneEntityCfg("vial"),
):
    """The orientation of the vial in the world frame."""
    vial: RigidObject = env.scene[vial_cfg.name]

    return vial.data.root_quat_w
...
def vial_euler_angles(
    env: ManagerBasedRLEnv,
    vial_cfg: SceneEntityCfg = SceneEntityCfg("vial"),
) -> torch.Tensor:
    """The euler angles of the vial in the world frame."""
    vial: RigidObject = env.scene[vial_cfg.name]

    return euler_xyz_from_quat(vial.data.root_quat_w)
...
def vial_obs(
    ...
    Vial (in world frame):
        ...
        vial quat,
        vial euler angles,
        gripper to vial vector,
        vial up vector (z-axis in world frame),
    """
    vial: RigidObject = env.scene[vial_cfg.name]
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]

    vial_pos_w = vial.data.root_pos_w
    vial_quat_w = vial.data.root_quat_w
    vial_euler = euler_xyz_from_quat(vial.data.root_quat_w)

    ee_pos_w = ee_frame.data.target_pos_w[:, 0, :]
    gripper_to_vial = vial_pos_w - ee_pos_w

    # Calculate vial up vector
    qx, qy, qz, qw = vial_quat_w[:, 0], vial_quat_w[:, 1], vial_quat_w[:, 2], vial_quat_w[:, 3]
    vial_up_vector = torch.stack([
        2 * (qx * qz + qy * qw),
        2 * (qy * qz - qx * qw),
        1 - 2 * (qx**2 + qy**2),
    ], dim=1,)
    ...
    return torch.cat(
        (
            vial_pos_w - env.scene.env_origins,
            vial_quat_w,
            vial_euler,
            gripper_to_vial,
            vial_up_vector,
...
def ee_frame_pos(env: ManagerBasedRLEnv, ee_frame_cfg: SceneEntityCfg = SceneEntityCfg("ee_frame")) -> torch.Tensor:
    """The position of the end effector in the world frame."""
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]
...
def ee_frame_quat(env: ManagerBasedRLEnv, ee_frame_cfg: SceneEntityCfg = SceneEntityCfg("ee_frame")) -> torch.Tensor:
    """The orientation of the end effector in the world frame."""
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]
...
def gripper_pos(env: ManagerBasedRLEnv, robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """The position of the gripper in the world frame."""
...
def vial_grasped(
    ...
    vial_cfg: SceneEntityCfg,
    diff_threshold: float = 0.06,
    gripper_open_val: torch.tensor = torch.tensor([0.04]),
    gripper_threshold: float = 0.005,
) -> torch.Tensor:
    """Check if a vial is grasped by the specified robot."""
    ...
    pose_diff = torch.linalg.vector_norm(vial_pos - end_effector_pos, dim=1)
